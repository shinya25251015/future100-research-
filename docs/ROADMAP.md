...
## Phase 5（市場規模・シナリオ）— 部分完了

シナリオ 3 本（撤回条件つき）は Phase 3-2 が生成する。残るのは TAM/SAM/SOM の推計で、
**これはコードではなく一次統計の取得が本体**のため未完了。ただし数値を受け入れる側の
規律は先に固定してある (`invariants.check_market_size`)。

- **出所のない市場規模を残さない** — 数値があるのに evidence が空なら弾く (§35-38)。
  未推計を 0 のまま置くのは正しい状態であり、違反ではない。
- **シナリオ間の順序を検査する** — 同じ年で bear <= base <= bull が壊れた予測は読めない (§9)。

一次統計の候補: 各国系統運用者の設備投資計画、業界団体の出荷統計、
政府統計 (IEA/EIA/Eurostat)。これらを Phase 2 のソースとして取り込んでから、
`market_size.method` に積み上げ方法を書いて埋める。
